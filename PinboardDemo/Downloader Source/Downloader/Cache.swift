...
open class Cache<T: DataConvertible> where T.Result == T, T : DataRepresentable {
    ...
    public init(name: String) {
        ...
        let originalFormat = Format<T>(name: DownloaderGlobals.Cache.OriginalFormatName)
    ...
    open func set(value: T, key: String, formatName: String = DownloaderGlobals.Cache.OriginalFormatName, success succeed: ((T) -> ())? = nil) {
    ...
    @discardableResult open func fetch(key: String, formatName: String = DownloaderGlobals.Cache.OriginalFormatName, failure fail : Fetch<T>.Failer? = nil, success succeed : Fetch<T>.Succeeder? = nil) -> Fetch<T> {
        ...
        if let (format, memoryCache, diskCache) = self.formats[formatName] {
            ...
            let error = errorWithCode(DownloaderGlobals.Cache.ErrorCode.formatNotFound.rawValue, description: description)
            fetch.fail(error)
        }
        return fetch
    }
    
    @discardableResult open func fetch(fetcher : Fetcher<T>, formatName: String = DownloaderGlobals.Cache.OriginalFormatName, failure fail : Fetch<T>.Failer? = nil, success succeed : Fetch<T>.Succeeder? = nil) -> Fetch<T> {
        let key = fetcher.key
        let fetch = Cache.buildFetch(failure: fail, success: succeed)
        self.fetch(key: key, formatName: formatName, failure: { error in
            if (error as NSError?)?.code == DownloaderGlobals.Cache.ErrorCode.formatNotFound.rawValue {
                fetch.fail(error)
            }
    ...
    open func remove(key: String, formatName: String = DownloaderGlobals.Cache.OriginalFormatName) {
        ...
        diskCache.fetchData(key: key, failure: { error in
            if let block = fail {
                if (error as NSError?)?.code == NSFileReadNoSuchFileError {
                    ...
                    let error = errorWithCode(DownloaderGlobals.Cache.ErrorCode.objectNotFound.rawValue, description: description)
                    block(error)
                } else {
                    block(error)
                }
    ...
    open func fetch(key: String, value getValue : @autoclosure @escaping () -> T.Result, formatName: String = DownloaderGlobals.Cache.OriginalFormatName, success succeed : Fetch<T>.Succeeder? = nil) -> Fetch<T> {
    ...
    open func fetch(path: String, formatName: String = DownloaderGlobals.Cache.OriginalFormatName,  failure fail : Fetch<T>.Failer? = nil, success succeed : Fetch<T>.Succeeder? = nil) -> Fetch<T> {
        let fetcher = DiskFetcher<T>(path: path)
        return self.fetch(fetcher: fetcher, formatName: formatName, failure: fail, success: succeed)
    }
    
    open func fetch(URL : Foundation.URL, formatName: String = DownloaderGlobals.Cache.OriginalFormatName,  failure fail : Fetch<T>.Failer? = nil, success succeed : Fetch<T>.Succeeder? = nil) -> Fetch<T> {
