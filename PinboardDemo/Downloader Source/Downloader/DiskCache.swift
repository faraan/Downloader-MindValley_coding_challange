...
open class DiskCache {
    ...
    open class func basePath() -> String {
        ...
        let hanekePathComponent = DownloaderGlobals.Domain
    ...
    open lazy var cacheQueue : DispatchQueue = {
        let queueName = DownloaderGlobals.Domain + "." + (self.path as NSString).lastPathComponent
    ...
    open func path(forKey key: String) -> String {
        ...
        let filename = escapedFilename.count < Int(NAME_MAX) ? escapedFilename : key.Filename()
