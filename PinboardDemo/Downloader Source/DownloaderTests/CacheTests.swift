...
class CacheTests: XCTestCase {
    ...
    func testFetchOnFailure_WithKey_ExpectAsyncFailure () {
        ...
        let fetch = sut.fetch(key: key).onFailure { error in
            ...
            XCTAssertEqual(error.domain, DownloaderGlobals.Domain)
            XCTAssertEqual(error.code, DownloaderGlobals.Cache.ErrorCode.objectNotFound.rawValue)
    ...
    func testFetch_WithKeyAndInexistingFormat_ExpectSyncFailure () {
        ...
        let fetch = sut.fetch(key: key, formatName: key, failure : { error in
            ...
            XCTAssertEqual(error.domain, DownloaderGlobals.Domain)
            XCTAssertEqual(error.code, DownloaderGlobals.Cache.ErrorCode.formatNotFound.rawValue)
            XCTAssertNotNil(error.localizedDescription)
            expectation.fulfill()
        }, success: { _ in
            XCTFail("expected failure")
