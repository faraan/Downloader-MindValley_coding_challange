...
class NetworkFetcherTests: XCTestCase {
    ...
    let URL = Foundation.URL(string: "http://Downloader.io/image.jpg")!
    ...
    func testFetchImage_Failure_InvalidData() {
        ...
        sut.fetch(failure: {
            XCTAssertEqual($0!._domain, DownloaderGlobals.Domain)
            XCTAssertEqual($0!._code, DownloaderGlobals.NetworkFetcher.ErrorCode.invalidData.rawValue)
    ...
    func testFetchImage_Failure_MissingData() {
        ...
        sut.fetch(failure: {
            XCTAssertEqual($0!._domain, DownloaderGlobals.Domain)
            XCTAssertEqual($0!._code, DownloaderGlobals.NetworkFetcher.ErrorCode.missingData.rawValue)
            XCTAssertNotNil($0!.localizedDescription)
            expectation.fulfill()
        }) { _ in
            XCTFail("expected failure")
    ...
    fileprivate func testFetchImageFailureWithInvalidStatusCode(_ statusCode : Int32) {
        ...
        sut.fetch(failure: {
            XCTAssertEqual($0!._domain, DownloaderGlobals.Domain)
            XCTAssertEqual($0!._code, DownloaderGlobals.NetworkFetcher.ErrorCode.invalidStatusCode.rawValue)
            XCTAssertNotNil($0!.localizedDescription)
            expectation.fulfill()
        }) { _ in
            XCTFail("expected failure")
    ...
    func testCacheFetch_Success() {
        ...
        _ = cache.fetch(URL: URL, failure: {_ in
            XCTFail("expected success")
            expectation.fulfill()
        }) {
    ...
    func testCacheFetch_WithFormat() {
        ...
        _ = cache.fetch(URL: URL, formatName: format.name, failure: {_ in
            expectation.fulfill()
        }) { _ in
            XCTFail("expected success")
            expectation.fulfill()
